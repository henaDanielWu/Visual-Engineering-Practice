...
def Panorama_stitching(image_right, image_left):
    ...
    if len(goodMatch) > 4:
        ...
        cv.imwrite("扭曲变换后的右图.jpg", Panorama)
...
if __name__ == '__main__':
    ...
    image_left=cv.rotate(image_left, cv.ROTATE_90_CLOCKWISE)
    image_right=cv.rotate(image_right, cv.ROTATE_90_CLOCKWISE)
    ...
    cv.imwrite("左图关键特征点检测.jpg", np.hstack((image_left, keypoints_image_left)))
    
    
    cv.imwrite("右图关键特征点检测.jpg", np.hstack((image_right, keypoints_image_right)))
    ...
    cv.imwrite("所有匹配的SIFT关键特征点连线.jpg", all_goodmatch_image)
